  ...
  return (
    <div className={`container w-full ${span !== 0 ? `col-span-${span}` : ''}`}>
      ...
      <div className='w-full flex items-center'>
        <input name={name} type={type === 'password' && reveal ? 'password' : 'text'} className={`border-2 border-slate-c1 rounded-md p-2 w-full mt-2 text-base ${externalStyle}`} required={required} readOnly={readonly} defaultValue={defaultValue}/>
        {
