        ...
        <div className={`container left-0 top-0 h-screen bg-primary w-[300px] py-6 flex flex-col justify-between transition-transfrom duration-500 ease-in sticky ${active ? 'w-[100px] transition-transfrom duration-500 ease-out' : ''}`}>
            ...
            <div className="grid place-items-center w-full gap-6 overflow-hidden mt-24">
                ...
                <motion.button onClick={() => navigate("/assignments")}
                    initial='buttonInitial'
                    animate={getCurrentLoc('/assignments') ? 'buttonAnimate' : ''}
                    variants={variants}
                    className={`${baseButton} ${getCurrentLoc('/assignments') ? 'bg-white' : 'bg-primary'}`}>
                    ...
                    <h3 className={`font-semibold text-white text-lg ml-4 ${getCurrentLoc('/assignments') ? 'text-primary' : 'text-white'}`}>Histori Kelulusan</h3>
                </motion.button>


                <motion.button onClick={() => navigate("/payments")}
                    initial='buttonInitial'
                    animate={getCurrentLoc('/payments') ? 'buttonAnimate' : ''}
                    variants={variants}
                    className={`${baseButton} ${getCurrentLoc('/payments') ? 'bg-white' : 'bg-primary'}`}
                >
                    <svg width="32" height="28" viewBox="0 0 32 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <motion.button onClick={() => navigate("/account")}
                    initial='buttonInitial'
                    animate={getCurrentLoc('/account') ? 'buttonAnimate' : ''}
                    variants={variants}
                    className={`${baseButton} ${getCurrentLoc('/account') ? 'bg-white' : 'bg-primary'}`}>
                    ...
                    <h3 className={`font-semibold text-white text-lg ml-4 ${getCurrentLoc('/account') ? 'text-primary' : 'text-white'}`}>Pengaturan Akun</h3>
                </motion.button>


                <motion.button onClick={() => navigate("/about")}
                    initial='buttonInitial'
                    animate={getCurrentLoc('/about') ? 'buttonAnimate' : ''}
                    variants={variants}
                    className={`${baseButton} ${getCurrentLoc('/about') ? 'bg-white' : 'bg-primary'}`}>
                    ...
                    <h3 className={`font-semibold text-white text-lg ml-4 ${getCurrentLoc('/about') ? 'text-primary' : 'text-white'}`}>Tentang Applikasi</h3>
                </motion.button>
            </div>

            <div className="grid place-items-center">
                <img src={Logo} alt="logo" className="w-[100px] h-[100px]" />
                <h2 className="font-bold text-2xl text-white mt-4">SIMAT</h2>
                <span className="font-semibold text-xl text-white">Dashboard</span>
            </div>
        </div>
